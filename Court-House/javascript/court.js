...
async function display_dialog_big(user, message, time) {
  speech.lang = "en-US";
  speech.text = message;
  speech.volume = 5;
  speech.rate = 1;
  ...
  document.getElementById("msg_big").innerHTML =
    `
<p>` +
    message +
    `</p>
`;
  setTimeout(function () {
    dialog_big.style.left = left + 70;
    dialog_big.style.top = top1 - 220;
    window.speechSynthesis.speak(speech);
    increaseOpacity(dialog_big, 0.01);
    scene++;
  }, 3000);
  await wait(3000);
  setTimeout(function () {
    decreaseOpacity(dialog_big, 0.03);
    scene++;
  }, time);
  await wait(time);
}
